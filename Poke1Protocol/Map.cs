...
namespace Poke1Protocol
{
    public class Map
    {
        ...
        public Dictionary<string, List<Tuple<int, int>>> AreaLinkDestinations { get; }

        public List<AreaLink> AreaLinks { get; }
        ...
        {
            AreaLinks = new List<AreaLink>();
            AreaLinkDestinations = new Dictionary<string, List<Tuple<int, int>>>();
            _client = client;
            ...
            TileTypes = (int[,])MapDump.TileTypes.ToArray<int>();
            TileTypes2 = (int[,])MapDump.TileTypes2.ToArray<int>();
            TileHeight = (int[,])MapDump.TileHeights.ToArray<int>();
            TileWater = (int[,])MapDump.TileWater.ToArray<int>();
            Colliders = (int[,])MapDump.Colliders.ToArray<int>();
            TileZones = (int[,])MapDump.TileZones.ToArray<int>();
            WallData = (HeightTilesStruct[,])MapDump.WallData.ToArray<HeightTilesStruct>();
            ...
            var dmX = GetPerfectDimensionX();
            var dmY = GetPerfectDimensionY();
            ...
            for (int y = 0; y < dmY; ++y)
            {
                for (int x = 0; x < dmX; ++x)
                {
                    if (IsAreaLink(x, y))
                    {
                        AreaLinks.Add(new AreaLink(CheckArea(x, y).AreaName, x, y));
                        if (!AreaLinkDestinations.ContainsKey(CheckArea(x, y).AreaName.ToUpperInvariant()))
                        {
                            AreaLinkDestinations.Add(CheckArea(x, y).AreaName.ToUpperInvariant(), new List<Tuple<int, int>>());
                        }
                        AreaLinkDestinations[CheckArea(x, y).AreaName.ToUpperInvariant()].Add(new Tuple<int, int>(x, y));
                    }
                }
            }
        ...
        public IEnumerable<Tuple<int, int>> GetNearestAreaLinks(string linkName, int x, int y)
        {
            if (AreaLinkDestinations.ContainsKey(linkName))
            {
                return AreaLinkDestinations[linkName].OrderBy(link => GameClient.DistanceBetween(x, y, link.Item1, link.Item2));
            }
            return null;
        }
        ...
        {
            return (x >= CurrentArea.StartX && x <= CurrentArea.EndX && y >= CurrentArea.StartY && y <= CurrentArea.EndY);
        }
        ...
        {
            if (IsInCurrentArea(x, y))
                return (TileTypes[x, y] == 1999 || TileTypes[x, y] == 1054 || TileTypes[x, y] != 8)
                && !HasLink(x, y) && TileTypes2[x, y] <= 0
